...
path1 <- "/mnt/4abcb8af-e2eb-47e2-86d6-eed71f6d5304/00_home_guests_do_not_move/fotisbs/Fotis2020/fotisfungi1a/lOTUs_moth_basis/" 
path2 <- "/mnt/4abcb8af-e2eb-47e2-86d6-eed71f6d5304/00_home_guests_do_not_move/fotisbs/Fotis2020/fotisfungi2a/lOTUs_moth_basis/"
path3 <- "/home/elepap/FOT1_2021/FOT1_2021_Fungi/demuxd_raw/fun/"
path4 <- "/home/elepap/FOT2_2021/FOT2_2021_Fungi/demuxd_raw/fun/"
...
write.table(track, file = "readQCFungi.txt", col.names = NA, sep = "\t", quote = FALSE)


##taxonomically classify the sequences##
taxa <-assignTaxonomy(seqs = seqtab.nochim, minBoot = 80, refFasta = "sh_general_release_dynamic_s_all_04.02.2020.fasta", multithread=TRUE, tryRC = TRUE)
...
VINE_ITS_All_2019.2020 <- phyloseq(otu_table(seqtab.nochim, 
                                             taxa_are_rows=FALSE),
                                   sample_data(samdf),
                                   tax_table(taxa))
...
sequences <- Biostrings::DNAStringSet(taxa_names(VINE_ITS_19.20))

names(sequences) <- taxa_names(VINE_ITS_19.20)

# The actual addition of the sequences is performed with the following command
ps <- merge_phyloseq(VINE_ITS_19.20, sequences)

VINE_ITS_All_2019.2020. <- ps

# the following command (uses the stringr::str_pad for padding leading zeros) performs the actual renaming
library(stringr)
taxa_names(VINE_ITS_All_2019.2020.) <- paste("ASV",str_pad(1:length(taxa_names(VINE_ITS_All_2019.2020.)),5, pad = "0"),sep = "")

rank_names(VINE_ITS_All_2019.2020.)

VINE_ITS_All_2019.2020.

##Create table,and check number of features for each phyla##
table(tax_table(VINE_ITS_All_2019.2020.)[, "Kingdom"], exclude = NULL)
table(tax_table(VINE_ITS_All_2019.2020.)[, "Phylum"], exclude = NULL) 
table(tax_table(VINE_ITS_All_2019.2020.)[, "Class"], exclude = NULL) 
table(tax_table(VINE_ITS_All_2019.2020.)[, "Order"], exclude = NULL)
table(tax_table(VINE_ITS_All_2019.2020.)[, "Family"], exclude = NULL)
table(tax_table(VINE_ITS_All_2019.2020.)[, "Genus"], exclude = NULL)
...
VINE_ITS_All_2019.2020. <- subset_taxa(VINE_ITS_All_2019.2020., Phylum %in% (c("p__Ascomycota", "p__Basidiomycota")))

##Removed not assigned (NA), uncharacterized Phylum##
VINE_ITS_All_2019.2020. <- subset_taxa(VINE_ITS_All_2019.2020., !is.na(Phylum) & !Phylum %in% (c("", "uncharacterized")))

VINE_ITS_All_2019.2020. <- prune_taxa(taxa_sums(VINE_ITS_All_2019.2020.)>0, VINE_ITS_All_2019.2020.)


##replace the NA names with the classified leftmost (higher level annotated) taxa##
VINE_ITS_All_2019.2020. <- VINE_ITS_All_2019.2020.


for(i in 1:nrow(tax_table(VINE_ITS_All_2019.2020.))){
  for(j in 2:ncol(tax_table(VINE_ITS_All_2019.2020.))){
    if(is.na(tax_table(VINE_ITS_All_2019.2020.)[i,j])){
      tax_table(VINE_ITS_All_2019.2020.)[i,j] <- tax_table(VINE_ITS_All_2019.2020.)[i,j-1]
    }
  }
}
...
saveRDS(VINE_ITS_All_2019.2020., file = "VINE_ITS_All_2019.2020..RDS")